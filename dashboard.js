...
// dashboard flash
//
window.page.ongetFlashDataSuccess = function(a) {
    ...
    this.FlashLine.setArea(true);
    this.FlashLine.setIsPeriodCompare(!!(this.context.st2 || this.context.et2))
};
...
// dashboard table
//
window.page.onConvertTableData = function(e) {
    var _mul_items = function(x) {
        if (isNaN(x)) {
            return x;
        } else {
            return x * MUL;
    ...
    var buf = [], buf2 = [], buf3 = [], buf4 = [];
    var c = e.items[0];

    e.items[1][0].forEach(function(x) {
        buf.push(_mul_items(x))
    });
    e.items[1][0] = buf;

    e.items[1][1].forEach(function(x) {
        buf2.push(_mul_items(x))
    });
    e.items[1][1] = buf2;
    
    e.items[2][0].forEach(function(x) {
        buf3.push(_mul_items(x))
    });
    e.items[2][0] = buf3;

    e.items[2][1].forEach(function(x) {
        buf4.push(_mul_items(x))
    });
    e.items[2][1] = buf4;

    for (var a in c) {
        var b = c[a][0],
            d = T.format('<span class="#{1}" title="!{0}"><span class="tb-label">!{0}</span>',
                    b.name, b.platformName.toLowerCase());
        if (!!b.isAuth) {
            d += '<span class="tb-label-auth">(权限APP)</span>'
        }
        d += "</span>";
        this.setIndexItem(b, d);
    }
};


window.page.showFlash = function(b, c) {

    if (b) {

        if (!this.context.initFlash) {
            this.context.initFlash = true;
            this.initComponents(["Date", "FlashFormatConverter", "Summary", {
                id: "FlashLine",
                type: "Flash",
                options: {
                    type: "line"
        ...
        c.sum[0].forEach( function (item) {
            item["val"] = item["val"] * MUL;
        });
        c.sum[1].forEach( function(item) {
            var itemMuled = item.replace(",", "") * MUL;

            if (itemMuled > 1000) {
                itemMuled = (itemMuled + "").split("").reverse().join("").match(/.{1,3}/g)
                    .join(",").split("").reverse().join("");
            }

            tempSum1.push(itemMuled);
        });
        c.sum[1] = tempSum1;

        this.Summary.render(c.sum, c.indicators);
        T.element(T.g("showf")).show();
        T.element(T.q("section")).show();
        T.element(T.g("ControlBarWrapper")).show();
        T.element(T.g("Paging")).hide();
        T.element(T.q("hint")).hide();
        if (this.context.changeView) {
            this.SortTypes.render(this.orderTypeList);
            this.context.order && this.SortTypes.select(this.context.order)
        }
        if (this.context.initFlash && this.context.changeView) {
            this.getFlashData()
        }
    } else {
        if (!this.context.initPaging) {
            this.context.initPaging = true;
            this.initComponents([{
                id: "Paging",
                type: "Paging",
                options: {
                    pageSize: this.context.pageSize,
                pageSizeRange: [this.context.pageSize]
                }
            }])
        }
        this.Paging.render(c.total, this.context.offset);
        if (this.context.changeView) {
            var a = this.context.order;
            if (a != "app_id,ASC" && a != "app_id,DESC") {
                a = ""
            }
            this.SortTypes.render([{
                id: "",
                label: " -- 请选择 -- ",
                style: {
                    display: "none"
            ...
            this.SortTypes.select(a)
        }
        T.element(T.g("showf")).hide();
        T.element(T.q("section")).hide();
        T.element(T.g("ControlBarWrapper")).hide();
        T.element(T.g("Paging")).show();
        T.element(T.q("hint")).show()
    }
    this.context.changeView = true
};
