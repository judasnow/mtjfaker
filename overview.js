...
//
// @TODO
// 1 启动的时候 sum 数字的跳动问题。应该是接管的时间点有问题。
// 2 修改 table 选项之后的 sum 部分刷新问题。
//
...
window.page.renderFlashArea = function() {
...
//
// overview Outline 最顶端的应用概况
//

window.page.oninit = function() {
    ...
    this.tempIndicator = T.array.find(this.indexInfo.indicators,
        function(e) {
            return e.id == "session_time_per_person"
        }
    );
    this.context.flashIndicators = this.indexInfo.flashSelected.join();
    this.context.outline = T.object.clone(this.outline);
    this.context.outline.fields = this.Indicator.map(this.outline.fields);
    this.context.outline.fields[0] = this.Indicator.index(this.outline.fields[0]);

    var tmpItems = [];

    this.context.outline["items"].forEach(function(innerItems) {
        var tmpInnerItems = [];
        innerItems.forEach(function(item) {
            // 这里的判断是因为 有些 item 是数字有些是复合结构 或者是
            // NaN
            // @TODO 小数部分是不用进行计算的
            if (! isNaN(item)) {
                tmpInnerItems.push(Math.floor(item * MUL));
            } else {
                if (! isNaN(item["val"])) {
                    item["val"] = item["val"] * MUL;
                }
                tmpInnerItems.push(item);
            }
        });
        tmpItems.push(tmpInnerItems);
    });
    this.context.outline["items"] = tmpItems;


    $("#Outline").html(AceTemplate.format("OutlineTemplate", this.context.outline));
    //T.insertHTML("Outline", "beforeEnd", AceTemplate.format("OutlineTemplate", this.context.outline));

    this.context.outlineSummary = T.object.clone(this.outlineSummary);
    this.context.outlineSummary.fields = this.Indicator.map(this.outlineSummary.fields);
    this.context.outlineSummary.fields[0].url = T.format("#{0}/activitydegree?appId=#{1}", this.systemConfig.baseUri, this.appInfo.id);
    this.context.outlineSummary.fields[1].url = T.format("#{0}/activitydegree?appId=#{1}", this.systemConfig.baseUri, this.appInfo.id);
    this.context.outlineSummary.fields[2].url = T.format("#{0}/retaineduser?appId=#{1}", this.systemConfig.baseUri, this.appInfo.id);
    this.context.outlineSummary.fields[3].url = T.format("#{0}/visit/frequency?appId=#{1}", this.systemConfig.baseUri, this.appInfo.id);


    var tmpItems = [];
    this.context.outlineSummary["items"].forEach(function(item) {
        tmpItems.push(Math.floor(item * MUL));
    });
    this.context.outlineSummary["items"] = tmpItems;

    $("#OutlineSummary").html(AceTemplate.format("OutlineSummaryTemplate", this.context.outlineSummary));
    //T.insertHTML("OutlineSummary", "beforeEnd", AceTemplate.format("OutlineSummaryTemplate", this.context.outlineSummary));

    // 右下角的自定义
    this.context.page = T.object.clone(this.page);
    this.context.page.fields = this.Indicator.map(this.page.fields);
    this.context.page.fields[0] = this.Indicator.index(this.page.fields[0]);
    this.context.page.fields[1] = this.Indicator.index(this.page.fields[1]);
    this.context.page["items"].forEach(function(item) {
        item[2] = item[2] * MUL;
        item[3] = item[3];
    })
    $("#Entrance").html(AceTemplate.format("EntranceTemplate", this.context.page));
    //T.insertHTML("Entrance", "beforeEnd", AceTemplate.format("EntranceTemplate", this.context.page));
    ...
    this.FlashIndicator = new T.ui.CheckGroup({
        ...
        limit: 1,
        onchange: function(e) {
            b.onchangeFlashIndicator(e)
        },
        autoRender: true
    });
    if (! (this.userInfo.roles.ico || this.userInfo.roles.admin) && (!T.config.memo || T.config.memo.NewFunTips !== 0) && T.config.newFunContent && T.config.newFunContent[0] && T.config.newFunContent[0].content) {
        var a = T.config.newFunContent[0].extraInfo;
        var d = a && a.popupHeight || 520,
            c = a && a.popupWidth || 750;
        ...
        T.on("popupClose", "click",
                function() {
                    b.NewFunctionsDialog.close()
                });
        T.on("popupKnown", "click",
                function() {
                    b.NewFunctionsDialog.close();
                    var e = (new Date()).getTime();
                    var f = window["bd_holmes" + e] = new Image();
                    f.src = T.config.systemConfig.memoUri + "?r=" + e + "&global=0&type=page-tip&status=0&pageId=" + T.config.pageInfo.id + "&appId=0&elementId=NewFunTips";
